...
void task0_AdSample(void)
{
	...
	static uint16_t filter_current[50],filter_voltage[50];
  ...
  if (pointer_adc<49)
	...
	uint32_t temp_current=0,temp_voltage=0;
	for(i=0;i<50;i++)
		 {
	...
	if (counter_adc==50)
	{
		...
		AdcQueue.ad_surge=ADCfilter[6]/200;
  ...
  /*end ad filter and save the result to AdcQueue*/
	AdcQueue.voltage=AdcQueue.ad_voltage*1259/1000-20;//unit is 0.1V
	AdcQueue.current = (AdcQueue.ad_current*38+2643)/100;//uint is 0.01A
