...
void heat_adjust(void)
{
	...
	if ((para.rPower<target_power)&&(counter_add==0)&&(counter_throttle==0)&&(para.rCurrent<(para.mCurrent-40)))
			    {
						if ((target_power-para.rPower)>300)
						{
							if (flag_jitter==0)
							{
								if ((target_power-para.rPower)>3000)
	...
	if (pwm_pulse>800)
	{
	if (pwm_pulse>para.mPulse)
		   pwm_pulse=para.mPulse;
    if (pulse_temp!=pwm_pulse)		
			{
				pulse_jitter=pwm_pulse;
				pulse_temp=pwm_pulse;
			}
		flag_jitter=1;//start to jitter frequency
		set_counter_pin_interrupt(1);	
	}	
	else
	{  
		flag_jitter=0;//stop jittering frequency
		set_counter_pin_interrupt(0);	
	}		
...
void heat_protect(void)
{
	...
	if (para.rCurrent>(para.mCurrent-10))//max current limitation
	{ 
		pwm_pulse_set(pwm_pulse--);
		ErrorReport(ERROR_H_CURRENT);
	}
	
	if (para.rCurrent>(para.mCurrent-30))//max current limitation
		ErrorReport(ERROR_H_CURRENT);
